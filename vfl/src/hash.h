...
inline void computeCuckoo(BT *aliceInput, BT *h1, BT *h2, BT *h3, cuckooEntry **_aliceCuckooTable) {
  ...
  for (size_t i = 0;i<BUCKETS;i++) {
	...
	*/
	...
	//At this point, we are holding one hash table entry e in our hand. 
  // We are now going to swap this entry with entries 
  // in the cuckoo table until we find an empty entry in the table
...
inline void computeHash(BT *bobInput, BT *h1, BT *h2, BT *h3, bucket *_bobTable, BT *bobTable, bucket *_bobPayloadTable, BT *bobPayloadTable) {
          uint64_t quotient = UNIVERSE/BUCKETS;
      ...
      BT xl = bobInput[i] / (quotient);
	    ...
	    //Mark which hash function we are using
            ...
            b->load++;
            // below are to compute payload,new
            bucket *b_payload = &_bobPayloadTable[index1];
            b_payload->values[b_payload->load] = (bobInput[i]+bobInput[i]+1);
            b_payload->load++;

            b_payload = &_bobPayloadTable[index2];
            b_payload->values[b_payload->load] = (HASHELEMENTS+bobInput[i]+bobInput[i]+2); 
            b_payload->load++;

            b_payload = &_bobPayloadTable[index3];
            b_payload->values[b_payload->load] = (2*HASHELEMENTS+bobInput[i]+bobInput[i]+3);
            b_payload->load++;
            // end new
    ...
    bucket *b_payload =  &_bobPayloadTable[i];
        ...
        // new
        if (j<b_payload->load) {
		bobPayloadTable[i*BETA+j] = b_payload->values[j]; 
	      } else {
		//Mark as invalid
		bobPayloadTable[i*BETA+j] = 3*HASHELEMENTS+1;
	      }
        //end new
            }
        }  
